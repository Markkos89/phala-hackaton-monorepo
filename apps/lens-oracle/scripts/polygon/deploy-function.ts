...
async function main() {
  ...
  const { output: brickProfileAddressQuery } =
    await brickProfileFactory.query.getUserProfileAddress<Result<AccountId, any>>(pair.address, {
      cert,
    });
  ...
  if (
    brickProfileCodeHash === "0x3b3d35f92494fe60d9f9f6139ea83964dc4bca84d7ac66e985024358c9c62969"
  ) {
  ...
  // estimate gas fee & storage deposit fee, check cluster balance enough or not.
  {
    ...
    const storageDepositeFee = !estimate.storageDeposit.isCharge
      ? 0
      : ((registry.clusterInfo?.depositPerByte?.toNumber() ?? 0) * ((source.length / 2) * 2.2)) /
        1e12;
  ...
  const { output: attestorQuery } = await contractPromise.query.getAttestAddress(cert.address, {
    cert,
  });
  ...
  const { output: numberQuery } = await brickProfile.query.workflowCount<u16>(pair.address, {
    cert,
  });
